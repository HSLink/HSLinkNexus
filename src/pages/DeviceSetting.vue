...
<template>
  <div class="min-h-screen p-6 bg-base-200">
    ...
    <div v-else class="space-y-6">
        ...
        <div class="card-body">
          ...
          <div class="form-control">
            <div class="relative">
              ...
              <input
                type="text"
                :placeholder="t('device_setting.nickname_placeholder')"
        ...
        <div class="card-body">
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="label-text text-sm opacity-70">{{
                    speed_boost_enable ? t('device_setting.enabled') : t('device_setting.disabled')
                  }}</span>
                </div>
              </div>
          ...
          <div class="grid md:grid-cols-2 gap-6">
            ...
            <div class="bg-base-200 p-4 rounded-lg">
              <label class="label justify-center">
                <span class="label-text text-lg font-medium">{{
                  t('device_setting.swd_output_mode')
                }}</span>
              </label>
              <div class="flex justify-center mt-2">
                <div class="flex gap-4">
                  <button
                    class="btn px-6"
                    :class="{
                      'btn-primary': swd_simulate_mode === 'spi',
                      'btn-outline': swd_simulate_mode !== 'spi',
                    }"
                    @click="swd_simulate_mode = 'spi'"
                  >
                    {{ t('device_setting.port_mode.spi') }}
                  </button>
                  <button
                    class="btn px-6"
                    :class="{
                      'btn-primary': swd_simulate_mode === 'gpio',
                      'btn-outline': swd_simulate_mode !== 'gpio',
                    }"
                    @click="swd_simulate_mode = 'gpio'"
                  >
                    {{ t('device_setting.port_mode.gpio') }}
                  </button>
                </div>
              </div>
            ...
            <div class="bg-base-200 p-4 rounded-lg">
              <label class="label justify-center">
                <span class="label-text text-lg font-medium">{{
                  t('device_setting.jtag_output_mode')
                }}</span>
              </label>
              <div class="flex justify-center mt-2">
                <div class="flex gap-4">
                  <button
                    class="btn px-6"
                    :class="{
                      'btn-primary': jtag_simulate_mode === 'spi',
                      'btn-outline': jtag_simulate_mode !== 'spi',
                    }"
                    @click="jtag_simulate_mode = 'spi'"
                  >
                    {{ t('device_setting.port_mode.spi') }}
                  </button>
                  <button
                    class="btn px-6"
                    :class="{
                      'btn-primary': jtag_simulate_mode === 'gpio',
                      'btn-outline': jtag_simulate_mode !== 'gpio',
                    }"
                    @click="jtag_simulate_mode = 'gpio'"
                  >
                    {{ t('device_setting.port_mode.gpio') }}
                  </button>
                </div>
          ...
          <div class="bg-base-200 p-4 rounded-lg">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                ...
                />
                <span class="label-text text-sm opacity-70">{{
                  jtag_20pin_compatible ? t('device_setting.enabled') : t('device_setting.disabled')
                }}</span>
              </div>
            </div>
            ...
            <div class="form-control bg-base-200 p-3 rounded-lg">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  />
                  <span class="label-text text-sm opacity-70">{{
                    power_power_on ? t('device_setting.enabled') : t('device_setting.disabled')
                  }}</span>
                </div>
              </div>
            ...
            <div class="form-control bg-base-200 p-3 rounded-lg">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  />
                  <span class="label-text text-sm opacity-70">{{
                    power_port_on ? t('device_setting.enabled') : t('device_setting.disabled')
                  }}</span>
                </div>
              </div>
          ...
          <div class="form-control mt-4">
            <div class="bg-base-200 p-4 rounded-lg">
              <div class="flex items-center justify-between mb-3">
                <span class="label-text text-lg font-medium">{{
                  t('device_setting.reference_voltage')
                }}</span>
                <div class="flex items-center gap-2">
              ...
              <transition name="fade" mode="out-in">
                <div v-if="!isExternalVref" key="internal" class="space-y-3">
                  ...
                  <div class="tabs tabs-boxed">
                    <a
                      class="tab"
                      :class="{ 'tab-active': voltageMode === 'preset' }"
                      @click="voltageMode = 'preset'"
                      >{{ t('device_setting.voltage_modes.preset') }}</a
                    >
                    <a
                      class="tab"
                      :class="{ 'tab-active': voltageMode === 'custom' }"
                      @click="switchToCustomMode"
                      >{{ t('device_setting.voltage_modes.custom') }}</a
                    >
                  </div>
                  ...
                  <transition name="fade" mode="out-in">
                    <div
                      v-if="voltageMode === 'preset'"
                      key="preset"
                      class="grid grid-cols-4 gap-2"
                    >
                      <button
                        v-for="voltage in presetVoltages"
                        :key="voltage"
                        class="btn"
                        :class="{
                          'btn-primary': power_vref_voltage === voltage,
                          'btn-outline': power_vref_voltage !== voltage,
                        }"
                      ...
                      <div class="relative">
                        <div class="text-center mb-2 pb-1 border-b border-base-300 opacity-70">
                          <span class="text-sm font-medium">{{
                            t('device_setting.voltage_input')
                          }}</span>
                        </div>
                        <div class="relative">
                          <input
                          ...
                          />
                          <span
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-base-content opacity-70"
                            >V</span
                          >
                        </div>
                      </div>
                        ...
                        <transition name="fade">
                          ...
                          >
                            <div class="flex">
                              ...
                              <div>
                                <strong>{{ t('device_setting.voltage_error') }}：</strong
                                >{{ voltageErrorMsg }}
                          ...
                          <div v-else class="text-xs mt-1 opacity-70 ml-1">
                            {{ t('device_setting.voltage_range_desc') }}
                          </div>
                        </transition>
                      </div>
                    </div>
                ...
                <div v-else key="external" class="bg-base-100 p-4 rounded-lg text-center">
                  <div class="text-lg font-medium">
                    {{ t('device_setting.external_vref_title') }}
                  </div>
                  <p class="text-xs mt-1 opacity-70">
                    {{ t('device_setting.external_vref_desc') }}
                  </p>
                </div>
              </transition>
        ...
        <div class="card-body">
          ...
          <div class="form-control">
            <div class="bg-base-200 p-4 rounded-lg">
              <label class="label">
                <span class="label-text text-lg font-medium">{{
                  t('device_setting.reset_modes')
                }}</span>
                <span class="text-xs opacity-70">{{ t('device_setting.reset_multi_select') }}</span>
              </label>
              <div class="grid md:grid-cols-3 gap-2 mt-2">
                <label
                  class="flex items-center gap-2 p-3 bg-base-100 rounded-lg cursor-pointer border-2 transition-all hover:bg-base-300"
                  :class="{
                    'border-primary': reset_mode.includes('nrst'),
                    'border-transparent': !reset_mode.includes('nrst'),
                  }"
                >
                  <input
                    type="checkbox"
                  ...
                  <div>
                    ...
                    <div class="text-xs opacity-70">
                      {{ t('device_setting.reset_mode.nrst_desc') }}
                    </div>
                  </div>
                </label>
                <label
                  class="flex items-center gap-2 p-3 bg-base-100 rounded-lg cursor-pointer border-2 transition-all hover:bg-base-300"
                  :class="{
                    'border-primary': reset_mode.includes('por'),
                    'border-transparent': !reset_mode.includes('por'),
                  }"
                >
                  <input
                    type="checkbox"
                  ...
                  <div>
                    ...
                    <div class="text-xs opacity-70">
                      {{ t('device_setting.reset_mode.por_desc') }}
                    </div>
                  </div>
                </label>
                <label
                  class="flex items-center gap-2 p-3 bg-base-100 rounded-lg cursor-pointer border-2 transition-all hover:bg-base-300"
                  :class="{
                    'border-primary': reset_mode.includes('arm_swd_soft'),
                    'border-transparent': !reset_mode.includes('arm_swd_soft'),
                  }"
                >
                  <input
                    type="checkbox"
                    ...
                    @change="
                      toggleResetMode('arm_swd_soft', ($event.target as HTMLInputElement).checked)
                    "
                  />
                  <div>
                    <div class="font-medium">{{ t('device_setting.reset_mode.arm_swd_soft') }}</div>
                    <div class="text-xs opacity-70">
                      {{ t('device_setting.reset_mode.arm_swd_soft_desc') }}
                    </div>
                  </div>
                </label>
              </div>
            ...
            <div class="form-control">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  />
                  <span class="label-text text-sm opacity-70">{{
                    led_enable ? t('device_setting.enabled') : t('device_setting.disabled')
                  }}</span>
                </div>
              </div>
            ...
            <div class="led-brightness-container">
              <transition name="slide-fade">
                <div v-if="led_enable" class="form-control mt-4 led-brightness-content">
                  <label class="label">
                    <span class="label-text text-lg font-medium">{{
                      t('device_setting.led_brightness')
                    }}</span>
                  </label>
                  <input
                    type="range"
      ...
      <!-- 保存按钮 -->
      <button
        class="btn btn-primary text-center w-full py-3 text-lg shadow-lg hover:scale-102 transition-all save-button"
