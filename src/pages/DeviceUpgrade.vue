  ...
  <div class="flex flex-col h-screen bg-gray-100 dark:bg-gray-800 p-6">
    ...
    <div v-if="!connected" class="flex items-center justify-center h-[70vh]">
      <div class="bg-base-200 bg-opacity-80 rounded-xl p-10 shadow-lg text-center max-w-lg">
        ...
        <p class="text-lg mb-6 text-base-content opacity-80">
          {{ $t('device_upgrade.connect_prompt') }}
        </p>
    ...
    <template v-else>
      <!-- Content Section -->
      <div
        class="flex flex-col lg:flex-row bg-white dark:bg-gray-700 shadow-md rounded-lg p-6 mb-4"
      >
        <!-- Left Column -->
        <div class="lg:w-1/3 space-y-4">
          ...
          <div class="space-y-2">
            <p class="text-lg">
              {{ $t('device_upgrade.bootloader.current_version') }}: {{ bl_ver }}
            </p>
          </div>
        </div>
        ...
        <div class="lg:w-2/3 space-y-4">
          <div class="space-y-2">
            <div v-if="connected">
              <input
                type="text"
                :placeholder="$t('device_upgrade.bootloader.firmware_path')"
                class="input input-bordered w-2/3"
                v-model="BootloaderFwPath"
              />
              <button class="btn btn-primary w-1/3" @click="SelectBootloaderFw">
                {{ $t('device_upgrade.bootloader.select_firmware') }}
              </button>
              <button
                class="btn btn-primary w-full mt-4"
                :disabled="!isAppVersionSupported"
                @click="UpgradeBootloader"
              >
                {{ $t('device_upgrade.bootloader.upgrade') }}
              </button>
      ...
      >
        ...
        <div class="lg:w-2/3 space-y-4">
          <div class="space-y-2">
            <div v-if="connected">
              <button class="btn btn-primary w-full mt-4" @click="EntryBL">
                {{ $t('device_upgrade.app.enter_bootloader') }}
              </button>
            </div>
            <div v-else-if="inBootloader">
              <div class="mb-4 flex items-center gap-4">
                <input
                  type="text"
                  :placeholder="$t('device_upgrade.app.firmware_path')"
                  class="input input-bordered w-2/3"
                  v-model="AppFwPath"
                />
                <button class="btn btn-primary w-1/3" @click="SelectAppFw">
                  {{ $t('device_upgrade.app.select_firmware') }}
                </button>
              </div>
              <button class="btn btn-primary w-full mt-4" @click="UpgradeApp">
                {{ $t('device_upgrade.app.upgrade') }}
              </button>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div class="lg:w-2/3 space-y-4">
          <div class="space-y-2">
            <button class="btn btn-primary w-full mt-4" @click="DownloadFw">
              {{ $t('device_upgrade.download.download_button') }}
            </button>
          </div>
        </div>
      </div>
